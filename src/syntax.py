...
TOKEN_TYPES = [
    "keyword", "builtin", "string", "comment", "number",
    "function", "class", "decorator", "operator", "punctuation",
    "variable", "constant", "type", "string_doc",
]
...
PYTHON_RULES = [
    ...
    (r'\b(?:False|None|True|and|as|assert|async|await|break|class|continue|def|del|'
     ...
     r'or|pass|raise|return|try|while|with|yield)\b', "keyword"),
    (r'\b(?:abs|all|any|bin|bool|bytearray|bytes|callable|chr|classmethod|compile|'
     ...
     r'staticmethod|str|sum|super|tuple|type|vars|zip|__import__)\b', "builtin"),
...
TYPESCRIPT_RULES = JAVASCRIPT_RULES + [
    (r'\b(?:interface|type|enum|as|any|boolean|number|string|void|null|undefined|'
     ...
     r'module|keyof|typeof|infer|satisfies)\b', "keyword"),
]
...
HTML_RULES = [
    ...
    (r'\b(?:class|id|style|src|href|rel|type|name|value|placeholder|disabled|'
     ...
     r'max|min|maxlength|minlength|pattern|readonly|required|spellcheck|step)\b', "builtin"),
    (r'[<>=/]', "operator"),
]
...
CSS_RULES = [
    ...
    (r'\b(?:[0-9]+(?:\.[0-9]*)?(?:px|em|rem|vh|vw|%|pt|cm|mm|in|ex|ch|vmin|vmax|deg|'
     r'rad|grad|turn|s|ms|Hz|kHz|dpi|dpcm|dppx)?)\b', "number"),
    (r'@[\w-]+', "decorator"),
    ...
    (r'\b(?:color|background|background-color|margin|padding|border|display|position|'
     ...
     r'page-break|orphans|widows)\b', "keyword"),
    (r'[{}:;,>~+*]', "punctuation"),
]
...
JAVA_RULES = [
    ...
    (r'\b(?:abstract|assert|boolean|break|byte|case|catch|char|class|const|continue|'
     ...
     r'transient|try|void|volatile|while)\b', "keyword"),
    ...
    (r'\b(?:String|Integer|Boolean|Double|Float|Long|Short|Byte|Character|Object|'
     ...
     r'Exception|RuntimeException|Error|Throwable)\b', "builtin"),
    (r'@\w+(?:\.\w+)*', "decorator"),
    (r'[+\-*/%&|^~]=?|==|!=|<=|>=|<|>|&&|\|\||!|\?', "operator"),
    (r'[\(\)\[\]\{\},;.]', "punctuation"),
]
...
C_RULES = [
    ...
    (r'#\s*include|#\s*define|#\s*ifdef|#\s*ifndef|#\s*if|#\s*else|#\s*elif|#\s*endif|'
     r'#\s*pragma|#\s*error|#\s*warning', "decorator"),
    (r'#.*$', "decorator"),
...
CSHARP_RULES = [
    ...
    (r'\b(?:abstract|as|base|bool|break|byte|case|catch|char|checked|class|const|'
     ...
     r'yield|record|init|required|file|scoped)\b', "keyword"),
    (r'\b(?:String|Int32|Int64|Boolean|Double|Single|Decimal|DateTime|TimeSpan|Guid|'
     ...
     r'StringBuilder|Array|var|dynamic)\b', "builtin"),
    (r'[+\-*/%&|^~]=?|==|!=|<=|>=|<|>|&&|\|\||!|\?|::|->|\?\?|=>', "operator"),
    (r'[\(\)\[\]\{\},;:]', "punctuation"),
]
...
RUST_RULES = [
    ...
    (r'\b(?:as|break|const|continue|crate|else|enum|extern|false|fn|for|if|impl|in|'
     ...
     r'do|final|macro|override|priv|try|typeof|unsized|virtual|yield)\b', "keyword"),
    (r'\b(?:i8|i16|i32|i64|i128|isize|u8|u16|u32|u64|u128|usize|f32|f64|bool|char|'
     ...
     r'Deref|Drop|Sized|Send|Sync|Unpin)\b', "type"),
...
PHP_RULES = [
    ...
    (r'\b(?:abstract|and|array|as|break|callable|case|catch|class|clone|const|'
     ...
     r'unset|use|var|while|xor|yield)\b', "keyword"),
...
RUBY_RULES = [
    ...
    (r'\b(?:BEGIN|END|alias|and|begin|break|case|class|def|defined\?|do|else|elsif|'
     ...
     r'self|super|then|true|undef|unless|until|when|while|yield)\b', "keyword"),
    (r'\b(?:attr_accessor|attr_reader|attr_writer|include|extend|prepend|require|'
     ...
     r'puts|raise|fail|sleep|loop|block_given\?)\b', "builtin"),
    (r'@\w+|@@\w+', "variable"),
    (r'[+\-*/%&|^~]=?|==|!=|<=|>=|<|>|&&|\|\||!|=>|::|\.\.|\.\.\.', "operator"),
    (r'[\(\)\[\]\{\},;]', "punctuation"),
]
...
SQL_RULES = [
    ...
    (r'\b(?:SELECT|FROM|WHERE|INSERT|INTO|VALUES|UPDATE|SET|DELETE|CREATE|TABLE|'
     ...
     r'CHAR|VARCHAR|TEXT|BLOB|ENUM|SET|BOOLEAN|BOOL|TRANSACTION)\b', "keyword"),
...
class SyntaxHighlighter:
    ...
    def highlight_visible(self):
        ...
        try:
            ...
            tokens: List[Tuple[int, int, int, str]] = []  # (abs_lineno, col_start, col_end, token_type)
            for rule_pattern, token_type in rules:
                ...
                for match in re.finditer(pattern, text, flags):
                    ...
                    if token_type in ("function", "class"):
                        ...
                        g_col_start = rel_gstart - g_last_newline - 1 if g_last_newline >= 0 else rel_gstart

                        g_span = text[:rel_gend]
                        g_last_newline_end = g_span.rfind('\n')
                        g_col_end = rel_gend - g_last_newline_end - 1 if g_last_newline_end >= 0 else rel_gend
            ...
            # Sort by line first, then by column start, then by token type priority
            ...
            # Apply tags per line, merging overlapping ranges
    ...
    def highlight_all(self):
        ...
        for rule_pattern, token_type in rules:
            for match in re.finditer(rule_pattern, text):
                ...
                last_newline = rel_before.rfind('\n')
                col_start = start_rel - last_newline - 1 if last_newline >= 0 else start_rel

                rel_span = text[:end_rel]
                last_newline_end = rel_span.rfind('\n')
                col_end = end_rel - last_newline_end - 1 if last_newline_end >= 0 else end_rel
                ...
                if token_type in ("function", "class"):
                    ...
                    g_last_newline = g_before.rfind('\n')
                    g_col_start = gstart - g_last_newline - 1 if g_last_newline >= 0 else gstart

                    g_span = text[:gend]
                    g_last_newline_end = g_span.rfind('\n')
                    g_col_end = gend - g_last_newline_end - 1 if g_last_newline_end >= 0 else gend
