...
class AISkillsExecutor:
    ...
    def __init__(
        ...
        self._skills = {
            ...
            "respond_in_chat": self._skill_respond_in_chat,
    ...
    def _skill_respond_in_chat(self, params: Dict[str, str]) -> AISkillResult:

        """Return a chat-only reply without modifying the editor."""

        content = params.get("content", "")

        if content is None:

            return AISkillResult(False, "No content provided for respond_in_chat")

        return AISkillResult(True, content)
    ...
    def parse_for_preview(self, response_text: str) -> List[tuple]:
        ...
        for match in matches:
            ...
            elif skill_name == "respond_in_chat":

                result = AISkillResult(True, params.get("content", ""))
    ...
    def apply_skill(self, skill_name: str, result: AISkillResult) -> AISkillResult:
        ...
        if skill_name in ("add_lines", "insert_lines"):
            ...
            return AISkillResult(True, result.message)
    ...
    def get_clean_response(self, response_text: str) -> str:
        ...
        cleaned = re.sub(
            ...
            r'<skill\s+name="respond_in_chat">\s*<parameter\s+name="content">(.*?)</parameter>\s*</skill>',
    ...
    def get_skill_description(self, skill_name: str) -> str:
        ...
        descriptions = {
            ...
            "respond_in_chat": "Return a conversational reply without changing the file",
...
You also have a chat-only skill for replies that should appear directly in the conversation without editing files.
Use <skill name="respond_in_chat"><parameter name="content">...</parameter></skill> when you want to answer in chat only.

    Be selective with context: inspect only the smallest set of relevant files and code blocks needed to complete the task.
    Prefer nearby definitions, direct callers, and focused tests over broad folder reads.
    If a file or section is not directly useful, do not read it.
...
<skill name="respond_in_chat"><parameter name="content">TEXT</parameter></skill>

    - Return a conversational reply directly in chat without changing any file.

    - Use this when the user wants an explanation, answer, or discussion instead of an edit.
